...
//
// Reads the Quake framebuffer of palette indices (R8) plus a single 256×1
// RGBA8 LUT, and produces an RGBA8 output. Replaces the CPU-side
// `palette_expand` loop in vid_sdl.c.
//
...
layout(set = 2, binding = 1) uniform sampler2D  u_palette;     // RGBA8 256×1
...
void main() {
    ...
    for (int j = 0; j < 4; ++j) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            acc += texelFetch(u_palette, ivec2(int(idx), 0), 0);
        }
    }
