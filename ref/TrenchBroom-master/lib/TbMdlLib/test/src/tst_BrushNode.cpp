...
TEST_CASE("BrushNode")
{
  ...
  SECTION("hasSelectedFaces")
  {
    ...
    auto brush = BrushNode{
      Brush::create(
        ...
        {
          // left
          createParaxial({0, 0, 0}, {0, 1, 0}, {0, 0, 1}),
          // right
          createParaxial({16, 0, 0}, {16, 0, 1}, {16, 1, 0}),
          // front
          createParaxial({0, 0, 0}, {0, 0, 1}, {1, 0, 0}),
          // back
          createParaxial({0, 16, 0}, {1, 16, 0}, {0, 16, 1}),
          // top
          createParaxial({0, 0, 16}, {0, 1, 16}, {1, 0, 16}),
          // bottom
          createParaxial({0, 0, 0}, {1, 0, 0}, {0, 1, 0}),
        })
      | kdl::value()};
      ...
      "selection")
    {
      ...
      auto copy = brush.brush();
      copy.face(0u).select();
      copy.face(1u).select();

      auto another = BrushNode{std::move(copy)};
    ...
    SECTION("patchNode")
    {
      ...
      SECTION("Brush does not contain any grid points, but patch intersects")
      {
        auto thinBrushNode = BrushNode{
          builder.createCuboid(vm::bbox3d{{1, -64, -64}, {2, 64, 64}}, "some_material")
          | kdl::value()};
  ...
  SECTION("pick")
  {
    ...
    brush.pick(editorContext, vm::ray3d({8, -8, 8}, {0, 1, 0}), hits1);
    CHECK(hits1.size() == 1u);
    ...
    brush.pick(editorContext, vm::ray3d({8, -8, 8}, {0, -1, 0}), hits2);
    CHECK(hits2.empty());
  }
...
TEST_CASE("BrushNode (Regression)", "[regression]")
{
  ...
  SECTION("1186")
  {
    /*
     ...
     This crash was caused by the correction of newly created vertices in
     Polyhedron::Edge::split - it would nudge vertices such that their plane status
     changed, resulting in problems when building the seam.
     */
  ...
  SECTION("1185")
  {
    /*
     ...
     The cause for the endless loop was, like above, the vertex correction in
  ...
  SECTION("1697")
  {
    ...
    /*
     This brush is broken beyond repair. When building the polyhedron, we run into
     problems where no seam can be computed. We opt to just throw an exception and expect
     it to fail without crashing.
     */
