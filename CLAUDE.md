...
- `edit_particle.c` — **Particle editor mode** (Phase: particle editor). ImGui panels (effect list, inspector, palette-swatch color-ramp editor, size envelope) over the data-driven emitter registry in `r_emitter.c`; preview spawns at the orbit-camera focus (RMB-drag to orbit, wheel/W,S to zoom, A,D to spin) with a play/pause clock independent of the paused world. The Preview controls include a **Hide world** toggle (`editor_particle_hide_world` cvar, default on): when set, `r_main.c` paints the 3D view over with `r_clearcolor` + zeroes the z-buffer span (`D_ClearViewToBackdrop` in `d_part.c`) right before the particle pass, so effects preview against a clean backdrop instead of the loaded map (gated by `Editor_ParticleHideWorld` — open + Particle mode only; normal play untouched). Effects persist as `id1/particles/*.pcl` (loaded/saved by `r_emitter.c`).
