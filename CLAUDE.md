...
- `edit_actor.c` — **Actor editor mode** (skeletal actors, E1 skeleton). Registered as `actor_mode` (third `s_modes[]` tab). Loads an IQM (`Mod_ForName`) and previews it with the shared orbit camera (`ActorMode_PushPreview` injects the actor entity into `cl_visedicts` at the orbit focus next to `Editor_PushPreviewEntities`; the R3 head look-at then tracks the orbit camera live). Read-only ImGui inspector (model/counts/roles + joint/mesh lists). Interactive authoring (box create/size/parent, joint rig, animation timeline, save via `lm_write_iqm`) is TBD and UX-heavy.
