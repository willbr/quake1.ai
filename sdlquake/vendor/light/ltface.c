...
typedef struct
{
	/* mono accumulator (per style, per sample) -- preserved from the
	 * vanilla path. The RGB accumulator below carries the same data with
	 * per-channel weighting; we keep both because (a) `add < 0` /
	 * `lightsamp > 1` heuristics in SingleLightFace read mono cleanly,
	 * and (b) the emit path falls back to mono if the resulting RGB
	 * triple under-flows to black (which would otherwise misclassify
	 * an unlit sample as lit). */
	vec_t	lightmaps[MAXLIGHTMAPS][SINGLEMAP];
	vec_t	lightmaps_rgb[MAXLIGHTMAPS][SINGLEMAP][3];
	int		numlightstyles;
	vec_t	*light;
	vec_t	facedist;
	vec3_t	facenormal;
...
// don't bother with light too far away
	...
	{	// init a new light map
		...
		{
			lightsamp[i] = 0;
			l->lightmaps_rgb[mapnum][i][0] = 0;
			l->lightmaps_rgb[mapnum][i][1] = 0;
			l->lightmaps_rgb[mapnum][i][2] = 0;
		}
	}
...
//
	...
	for (c=0 ; c<l->numsurfpt ; c++, surf+=3)
	{
		...
		if (add < 0)
			continue;
		lightsamp[c] += add;
		l->lightmaps_rgb[mapnum][c][0] += add * light->color[0];
		l->lightmaps_rgb[mapnum][c][1] += add * light->color[1];
		l->lightmaps_rgb[mapnum][c][2] += add * light->color[2];
...
// if minlight is set, there must be a style 0 light map
	...
	if (i == l->numlightstyles)
	{
		...
		for (j=0 ; j<l->numsurfpt ; j++)
		{
			...
			/* Mono minlight applies as neutral white tint. */
			l->lightmaps_rgb[i][j][0] = minlight;
			l->lightmaps_rgb[i][j][1] = minlight;
			l->lightmaps_rgb[i][j][2] = minlight;
		}
		l->lightstyles[i] = 0;
		l->numlightstyles++;
	}
	else
	{
		for (j=0 ; j<l->numsurfpt ; j++)
		{
			if (l->lightmaps[i][j] < minlight)
				l->lightmaps[i][j] = minlight;
			if (l->lightmaps_rgb[i][j][0] < minlight)
				l->lightmaps_rgb[i][j][0] = minlight;
			if (l->lightmaps_rgb[i][j][1] < minlight)
				l->lightmaps_rgb[i][j][1] = minlight;
			if (l->lightmaps_rgb[i][j][2] < minlight)
				l->lightmaps_rgb[i][j][2] = minlight;
		}
	}
}
...
//
	...
	/* The RGB sidecar buffer is laid out 1:1 with dlightdata, three
	 * bytes per mono byte. By keying off (out - filebase) we get the
	 * same per-face offset arithmetic for free. */
	out_rgb = light_rgb_dlightdata + (out - filebase) * 3;
...
// extra filtering
	...
	for (i=0 ; i< l.numlightstyles ; i++)
	{
		...
		light     = l.lightmaps[i];
		light_rgb = l.lightmaps_rgb[i];
		c = 0;
		for (t=0 ; t<=l.texsize[1] ; t++)
			for (s=0 ; s<=l.texsize[0] ; s++, c++)
			{
				if (extrasamples)
				{	// filtered sample (mono)
					total = light[t*2*w+s*2] + light[t*2*w+s*2+1]
					+ light[(t*2+1)*w+s*2] + light[(t*2+1)*w+s*2+1];
					total *= 0.25;
					/* Same 2x2 box filter for each RGB channel. */
					total_rgb[0] = (light_rgb[t*2*w+s*2][0] + light_rgb[t*2*w+s*2+1][0]
					              + light_rgb[(t*2+1)*w+s*2][0] + light_rgb[(t*2+1)*w+s*2+1][0]) * 0.25;
					total_rgb[1] = (light_rgb[t*2*w+s*2][1] + light_rgb[t*2*w+s*2+1][1]
					              + light_rgb[(t*2+1)*w+s*2][1] + light_rgb[(t*2+1)*w+s*2+1][1]) * 0.25;
					total_rgb[2] = (light_rgb[t*2*w+s*2][2] + light_rgb[t*2*w+s*2+1][2]
					              + light_rgb[(t*2+1)*w+s*2][2] + light_rgb[(t*2+1)*w+s*2+1][2]) * 0.25;
				}
				else
				{
					total = light[c];
					total_rgb[0] = light_rgb[c][0];
					total_rgb[1] = light_rgb[c][1];
				...
				if (total < 0) total = 0;
				if (total_rgb[0] > 255) total_rgb[0] = 255;
				if (total_rgb[1] > 255) total_rgb[1] = 255;
				if (total_rgb[2] > 255) total_rgb[2] = 255;
				if (total_rgb[0] < 0) total_rgb[0] = 0;
				if (total_rgb[1] < 0) total_rgb[1] = 0;
				if (total_rgb[2] < 0) total_rgb[2] = 0;

				*out++ = total;
				out_rgb[0] = (byte)total_rgb[0];
				out_rgb[1] = (byte)total_rgb[1];
				out_rgb[2] = (byte)total_rgb[2];
				out_rgb += 3;
			}
	}
